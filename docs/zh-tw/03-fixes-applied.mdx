...
這些修正調整 OpenAPI 結構描述約束，使規格與即時 API 實際執行的行為一致。每項修正由[驗證](/api-specs/zh-tw/02-pipeline/#2-validate)過程中產生的 `Discrepancy` 物件驅動。
...
這些修正處理由 [Spectral 檢查](/api-specs/zh-tw/04-spectral-rules/)發現的 OAS3 品質問題。它們新增或修正中繼資料，不會改變 API 行為。
...
伺服器 URL 和變數值來自 [validation.yaml](/api-specs/zh-tw/05-configuration/) 中的 `spectral.servers`。
...
值來自 [validation.yaml](/api-specs/zh-tw/05-configuration/) 中的 `spectral.contact`。
...
方案定義來自 [validation.yaml](/api-specs/zh-tw/05-configuration/) 中的 `spectral.security_scheme`。這告訴 API 使用者如何進行身份驗證，但不會改變執行時行為。
