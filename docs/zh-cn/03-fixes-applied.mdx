...
这些修复调整 OpenAPI 模式约束，使规范与实际 API 的执行行为保持一致。每项修复由[验证](/api-specs/zh-cn/02-pipeline/#2-validate)阶段产生的 `Discrepancy` 对象驱动。
...
:::caution
约束修复会更改 API 契约。仅在有来自 Schemathesis 或针对实际 API 进行约束验证测试的证据支持时，才会应用这些修复。
:::

### 支持的约束类型

测试和修复的 10 种 OpenAPI 约束类别：
...
这些修复解决由 [Spectral 检查](/api-specs/zh-cn/04-spectral-rules/)发现的 OAS3 质量问题。它们添加或修正元数据，而不改变 API 行为。
...
服务器 URL 和变量值来自 [validation.yaml](/api-specs/zh-cn/05-configuration/) 中的 `spectral.servers`。
...
值来自 [validation.yaml](/api-specs/zh-cn/05-configuration/) 中的 `spectral.contact`。
...
安全方案定义来自 [validation.yaml](/api-specs/zh-cn/05-configuration/) 中的 `spectral.security_scheme`。这告知 API 消费者如何进行身份验证，但不改变运行时行为。
