...
| `output_dir` | 解压后规范文件的存储位置。 |
| `etag_cache` | 存储 HTTP ETag 用于条件下载的文件。 |

### 验证类别

十个类别对应 OpenAPI 模式约束类型。每个类别都可以独立启用/禁用。
...
| `max_examples` | 每个操作生成的最大测试用例数量。 |
| `hypothesis_phases` | 要运行的 Hypothesis 阶段（`generate`、`target`）。 |
| `stateful_testing` | 启用跨操作的有状态链接测试。 |
| `base_url_override` | 覆盖测试使用的 API 基础 URL。`null` 表示使用 `api.base_url`。 |
...
| 字段 | 描述 |
|-------|-------------|
| `enabled` | 完全开关 Spectral 检查。 |
| `ruleset` | 使用的 Spectral 配置文件（流水线使用 `spectral-pipeline.yaml`）。 |
| `auto_fix` | 规则名称到布尔值的映射。`true` 表示协调器将修复违规项。 |
| `gate.max_errors` | 协调后门禁中允许的最大错误数。`null` 表示禁用该检查。 |
| `gate.max_warnings` | 允许的最大警告数。`null` 表示禁用该检查。 |
...
定义用于实时 API 验证的基准端点。每个条目将一个逻辑资源名称映射到其规范文件、API 组和 CRUD 路径。
...
:::tip
无论 `endpoints.yaml` 中列出了什么内容，框架在协调过程中都会验证全部 268 个规范。端点配置控制哪些资源使用 Schemathesis 进行实时 API 测试。
:::
