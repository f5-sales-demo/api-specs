...
Ces correctifs ajustent les contraintes de schéma OpenAPI afin que la spécification corresponde à ce que l'API en production applique réellement. Chaque correctif est piloté par un objet `Discrepancy` produit lors de la [validation](/api-specs/en/02-pipeline/#2-validate).
...
Ces correctifs traitent les problèmes de qualité OAS3 détectés par le [linting Spectral](/api-specs/en/04-spectral-rules/). Ils ajoutent ou corrigent des métadonnées sans modifier le comportement de l'API.
...
L'URL du serveur et les valeurs des variables proviennent de `spectral.servers` dans [validation.yaml](/api-specs/en/05-configuration/).
...
Les valeurs proviennent de `spectral.contact` dans [validation.yaml](/api-specs/en/05-configuration/).
...
La définition du schéma provient de `spectral.security_scheme` dans [validation.yaml](/api-specs/en/05-configuration/). Cela indique aux consommateurs de l'API comment s'authentifier, mais ne modifie pas le comportement à l'exécution.
