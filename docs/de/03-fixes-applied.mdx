...
Diese Korrekturen passen OpenAPI-Schema-Constraints an, damit die Spezifikation dem entspricht, was die Live-API tatsächlich durchsetzt. Jede Korrektur wird durch ein `Discrepancy`-Objekt gesteuert, das während der [Validierung](/api-specs/de/02-pipeline/#2-validate) erzeugt wird.
...
Diese Korrekturen beheben OAS3-Qualitätsprobleme, die durch [Spectral-Linting](/api-specs/de/04-spectral-rules/) gefunden wurden. Sie fügen Metadaten hinzu oder korrigieren diese, ohne das API-Verhalten zu ändern.
...
Die Server-URL und Variablenwerte stammen aus `spectral.servers` in [validation.yaml](/api-specs/de/05-configuration/).
...
Die Werte stammen aus `spectral.contact` in [validation.yaml](/api-specs/de/05-configuration/).
...
Die Schema-Definition stammt aus `spectral.security_scheme` in [validation.yaml](/api-specs/de/05-configuration/). Dies informiert API-Konsumenten über die Authentifizierung, ändert jedoch nicht das Laufzeitverhalten.
