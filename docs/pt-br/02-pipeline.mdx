...
As violações são convertidas em objetos `Discrepancy` e escritas em `reports/spectral_report.json`. Consulte [Regras do Spectral](/api-specs/pt-br/04-spectral-rules/) para o conjunto completo de regras.

### 4. Reconciliação

`scripts/reconcile.py` consome ambos os arquivos de relatório e aplica correções em cada especificação:

- **Correções de restrições** ajustam as restrições de esquema (relaxar, restringir, adicionar, remover) com base no comportamento observado da API. Consulte [Correções Aplicadas](/api-specs/pt-br/03-fixes-applied/).
