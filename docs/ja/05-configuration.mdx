---
title: 設定リファレンス
description: validation.yaml および endpoints.yaml 設定ファイルのリファレンス
i18n:
...
| フィールド | 説明 |
|-------|-------------|
| `base_url` | F5 XCコンソールAPIのベースURL。`F5XC_API_URL` 環境変数で上書き可能。 |
| `tenant` | APIパスで使用されるテナント名。 |
| `namespace` | CRUD操作のデフォルトネームスペース。 |
| `timeout` | HTTPリクエストのタイムアウト（秒）。 |
| `retries` | 失敗時のリトライ回数。 |
| `retry_delay` | リトライ間の秒数。 |

:::caution
`validation.yaml` の `base_url`、`tenant`、`namespace` の値は汎用的なプレースホルダーです。パイプラインを実行する前に、環境変数を使用してお使いの環境に合わせて**必ず**設定してください：
...
OpenAPIスキーマの制約タイプに対応する10のカテゴリ。それぞれ個別に有効/無効を切り替えることができます。
...
| フィールド | 説明 |
|-------|-------------|
| `priority` | 複数のデータソースが一致しない場合の優先順位。 |
...
| `ruleset` | 使用するSpectral設定ファイル（パイプラインは `spectral-pipeline.yaml` を使用）。 |
| `auto_fix` | ルール名からブール値へのマップ。`true` の場合、リコンサイラーが違反を修正。 |
| `gate.max_errors` | リコンシリエーション後のゲートで許容される最大エラー数。`null` でチェックを無効化。 |
| `gate.max_warnings` | 許容される最大警告数。`null` でチェックを無効化。 |
| `contact` | `info-contact` フィクサーによって `info.contact` に挿入される連絡先情報。 |
| `servers` | `oas3-api-servers` フィクサーによって挿入されるサーバー配列。 |
| `security_scheme` | すべてのスペックに挿入されるセキュリティスキーム定義。 |
...
| `version_from` | バージョンのソース。`git` はスペックのメタデータ日付 + パッチ番号からバージョンを導出。 |

## `config/endpoints.yaml`

ライブAPIバリデーションに使用されるベースラインエンドポイントを定義します。各エントリは論理リソース名をスペックファイル、APIグループ、およびCRUDパスにマッピングします。
...
| `api_group` | APIグループのプレフィックス（`config`、`web` など）。 |
| `crud_operations` | 各CRUD操作のHTTPメソッドとパス。 |
| `test_priority` | `high`、`medium`、または `low` -- テスト実行の優先順位を制御。 |
| `description` | リソースの人間が読める説明。 |

### 現在のベースラインエンドポイント

3つのドメインにわたる10のエンドポイント：
...
バリデーション用の新しいエンドポイントを追加するには：

1. `specs/original/` でスペックのファイル名を見つけます（形式：`docs-cloud-f5-com.NNNN.*.ves-swagger.json`）
2. 上記の構造に従って `endpoints:` の下にエントリを追加します
3. `domain_files:` の下に優先度番号付きでドメインファイルのマッピングを追加します
4. 希望する位置の `test_order:` にエンドポイント名を追加します

:::tip
フレームワークは `endpoints.yaml` に記載されている内容に関係なく、リコンシリエーション中に全268スペックをバリデーションします。エンドポイント設定は、SchemathesisによるライブAPIテストの対象となるリソースを制御します。
:::
