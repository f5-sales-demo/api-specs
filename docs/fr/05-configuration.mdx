...
### Entrée de validation en direct
...
Les tests en direct résolvent leurs domaines sémantiques à partir de la sortie de transformation corrigée.
Les fichiers bruts dans `specs/original/` restent les octets d'atterrissage en amont immuables et
ne sont jamais présentés comme le contrat d'API intermédiaire corrigé.
...
| `fix_strategies` | Associe chaque type de divergence à son action corrective. Voir [Corrections appliquées](/api-specs/en/03-fixes-applied/). |
