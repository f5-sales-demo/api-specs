...
from scripts.utils.reconciliation_report import ReconciliationReportError, validate_reconciliation_report
...
def setup_reconcile_env(monkeypatch, tmp_path):
    ...
    # We must copy config/reconciliation_report.schema.json to the expected relative path from python execution
    # since reconciliation_report.py looks for it at Path(__file__).resolve().parents[2] / "config" / ...
    # Wait, reconciliation_report.py uses Path(__file__).resolve() which is the absolute path inside scripts/utils/.
    # Since we are running in the repo, the schema path resolve will be correct automatically.
        ...
        if not simulate_missing_report:
            if discrepancies is not None:
                if isinstance(discrepancies, list):
                    ...
                    # Allow writing raw invalid dicts/structures
...
def test_reconcile_zero_fixes(setup_reconcile_env):
    ...
    # Validate via our shared contract validator
...
def test_reconcile_successful_fix(setup_reconcile_env):
    ...
    # Every fix preserves full source provenance
...
def test_reconcile_validation_rollback(setup_reconcile_env):
    ...
    ret = run(discrepancies, specs)
    # The rollback correctly causes validation failures, which returns nonzero (1)
    assert ret == 1
...
def test_unsupported_strategy_fails_apply(setup_reconcile_env):
    ...
    # Strategy 'skip' for extra_constraint if no fix strategies defined (default is skip / apply failure)
    # Wait, extra_constraint strategy default is 'remove'. Let's trigger a no-op mutation on relax constraint:
    # already relaxed to 10 (api_behavior == 10)
    discrepancies = [{
        "spec_file": "test_spec.json",
        "path": "TestModel",
        "property_name": "TestModel",
