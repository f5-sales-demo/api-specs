...
Queste correzioni regolano i vincoli degli schemi OpenAPI affinché la specifica corrisponda a ciò che l'API live effettivamente applica. Ogni correzione è guidata da un oggetto `Discrepancy` prodotto durante la [validazione](/api-specs/en/02-pipeline/#2-validate).
...
Queste correzioni risolvono problemi di qualità OAS3 individuati dal [linting Spectral](/api-specs/en/04-spectral-rules/). Aggiungono o correggono metadati senza modificare il comportamento dell'API.
...
L'URL del server e i valori delle variabili provengono da `spectral.servers` in [validation.yaml](/api-specs/en/05-configuration/).
...
I valori provengono da `spectral.contact` in [validation.yaml](/api-specs/en/05-configuration/).
...
La definizione dello schema proviene da `spectral.security_scheme` in [validation.yaml](/api-specs/en/05-configuration/). Questo indica ai consumatori dell'API come autenticarsi, ma non modifica il comportamento a runtime.
