---
title: 設定參考
description: validation.yaml 和 endpoints.yaml 設定檔案的參考文件
i18n:
...
| 欄位 | 說明 |
|-------|-------------|
| `base_url` | F5 XC 主控台 API 基底 URL。可透過 `F5XC_API_URL` 環境變數覆寫。 |
| `tenant` | 用於 API 路徑的租戶名稱。 |
| `namespace` | CRUD 操作的預設命名空間。 |
| `timeout` | HTTP 請求逾時時間（秒）。 |
| `retries` | 失敗時的重試次數。 |
| `retry_delay` | 重試之間的間隔秒數。 |

:::caution
`validation.yaml` 中的 `base_url`、`tenant` 和 `namespace` 值為通用佔位符。您**必須**在執行管線之前透過環境變數為您的環境進行設定：
...
| 欄位 | 說明 |
|-------|-------------|
| `url` | 官方 F5 XC OpenAPI 規格套件（ZIP）的 URL。 |
| `output_dir` | 解壓縮後規格檔案的儲存位置。 |
| `etag_cache` | 儲存 HTTP ETag 以進行條件式下載的檔案。 |

### 驗證類別

十個類別對應 OpenAPI 結構描述約束類型。每個類別可獨立啟用/停用。
...
| 欄位 | 說明 |
|-------|-------------|
| `enabled` | 切換 Schemathesis 測試的啟用/停用。 |
| `max_examples` | 每個操作生成的最大測試案例數量。 |
| `hypothesis_phases` | 要執行的 Hypothesis 階段（`generate`、`target`）。 |
| `stateful_testing` | 啟用跨操作的有狀態連結式測試。 |
| `base_url_override` | 覆寫測試用的 API 基底 URL。`null` 使用 `api.base_url`。 |
| `auth_header` | 用於身份驗證的 HTTP 標頭名稱。 |
| `auth_prefix` | 權杖前綴格式（請求以 `APIToken <token>` 格式發送）。 |
...
| `fix_strategies` | 將每種差異類型對應到其修復動作。請參閱[已套用的修復](/03-fixes-applied/)。 |
...
| 欄位 | 說明 |
|-------|-------------|
| `enabled` | 完全切換 Spectral 檢查的啟用/停用。 |
| `ruleset` | 使用的 Spectral 設定檔（管線使用 `spectral-pipeline.yaml`）。 |
| `auto_fix` | 規則名稱對布林值的對應。`true` 表示調和器將修復違規項目。 |
| `gate.max_errors` | 調和後閘門允許的最大錯誤數。`null` 停用此檢查。 |
| `gate.max_warnings` | 允許的最大警告數。`null` 停用此檢查。 |
| `contact` | 由 `info-contact` 修復器注入到 `info.contact` 的聯絡資訊。 |
| `servers` | 由 `oas3-api-servers` 修復器注入的伺服器陣列。 |
...
| `version_from` | 版本來源。`git` 從規格中繼資料日期加上修補號碼推導版本。 |

## `config/endpoints.yaml`

定義用於即時 API 驗證的基準端點。每個條目將邏輯資源名稱對應到其規格檔案、API 群組和 CRUD 路徑。
...
2. 在 `endpoints:` 下按照上述結構新增條目
3. 在 `domain_files:` 下新增領域檔案對應及優先順序編號
4. 將端點名稱新增到 `test_order:` 中所需的位置

:::tip
無論 `endpoints.yaml` 中列出了什麼，框架在調和過程中都會驗證所有 268 個規格。端點設定控制的是哪些資源會使用 Schemathesis 進行即時 API 測試。
:::
