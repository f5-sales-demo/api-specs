...
Live-Tests lösen ihre semantischen Domänen aus der korrigierten Transformationsausgabe auf.
Rohdateien in `specs/original/` bleiben die unveränderlichen Upstream-Landing-Bytes und
werden niemals als korrigierter Zwischen-API-Vertrag dargestellt.
...
| `fix_strategies` | Ordnet jeden Diskrepanztyp seiner Korrekturmaßnahme zu. Siehe [Angewandte Korrekturen](/api-specs/en/03-fixes-applied/). |
