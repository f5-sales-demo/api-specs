...
# Spectral の依存関係をインストール（リンティングステージに必要）
npm ci --ignore-scripts
```

`make dev-install` は `.venv/` に仮想環境を作成し、開発用エクストラ（pytest、ruff、mypy）を含むプロジェクトをインストールします。
...
3. **修正メソッドを追加** -- `scripts/reconcile.py` の `SpecReconciler` にメソッドを追加します：
   ```python
...
4. **登録する** -- `_apply_spectral_fix` ディスパッチャーに追加します：
   ```python
   spectral_fixers = {
       # ... 既存の修正メソッド ...
...
:::caution
カスタム関数は `spectral-pipeline.yaml` でのみ動作します。`.spectral.yaml` に `functionsDir` や `functions` を追加しないでください -- その設定は Super-Linter やその他の外部ツールとの互換性を維持する必要があります。
:::
