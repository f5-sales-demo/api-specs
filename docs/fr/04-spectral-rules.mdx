---
title: Règles de linting Spectral
description: >-
  Architecture Spectral à deux configurations avec des règles OAS de base et une
  fonction personnalisée f5-path-params
i18n:
  sourceHash: bff762289470
  translator: machine
---

Le projet utilise [Spectral](https://stoplight.io/open-source/spectral) pour le linting OAS3 avec une architecture à deux configurations qui sépare le CI/outillage externe de la logique personnalisée du pipeline.
...
Ces règles sont auto-corrigées par le réconcilieur lorsque des violations sont détectées. Voir [Corrections appliquées](/api-specs/fr/03-fixes-applied/) pour les détails sur chaque correction.
...
:::note
`f5-path-params` est une règle de sévérité error mais n'est pas auto-corrigée. Les violations indiquent un véritable défaut de spécification qui nécessite une revue manuelle.
:::
