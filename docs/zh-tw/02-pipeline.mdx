---
title: 管線概覽
description: 六階段自動化管線，負責下載、驗證、靜態分析、調和、品質閘門及發布 F5 XC OpenAPI 規格
i18n:
  sourceHash: 4dfb349b9347
  translator: machine
---

此管線透過 GitHub Actions（`validate-and-release.yml`）每日於 UTC 早上 6 點執行，並在每次推送至 `main` 分支且涉及 `scripts/`、`config/` 或 `pyproject.toml` 的變更時觸發。也可以手動觸發，並提供強制發布或跳過即時 API 測試的選項。
...
`scripts/download.py` 從 `docs.cloud.f5.com` 取得官方 F5 XC OpenAPI 規格 ZIP 檔案。它使用 HTTP ETag 標頭進行條件式 GET 請求——若上游套件未變更，則跳過下載。解壓縮後的 JSON 規格檔案存放於 `specs/original/`。
...
違規項目會轉換為 `Discrepancy` 物件並寫入 `reports/spectral_report.json`。完整規則集請參閱 [Spectral 規則](/api-specs/zh-tw/04-spectral-rules/)。

### 4. 調和

`scripts/reconcile.py` 消費兩份報告檔案，並對每個規格套用修正：

- **約束修正**根據觀察到的 API 行為調整結構描述約束（放寬、收緊、新增、移除）。請參閱[已套用的修正](/api-specs/zh-tw/03-fixes-applied/)。
- **Spectral 修正**為規格補充 servers、聯絡資訊、標籤、安全性中繼資料，並移除無用的元件。
- **安全性中繼資料**在 `security_scheme` 設定存在時一律注入，即使 Spectral 未標記問題。

修正後的規格寫入 `release/specs/`，並在儲存前使用 `openapi-spec-validator` 進行驗證。若修正後的規格驗證失敗，則使用原始規格作為備援。

### 5. Spectral 品質閘門

`scripts/spectral_lint.py --mode gate` 針對 `release/specs/` 中已調和的規格重新執行 Spectral。品質閘門會根據可設定的閾值（`validation.yaml` 中的 `spectral.gate.max_errors` 和 `spectral.gate.max_warnings`）檢查錯誤和警告數量。

:::note
第一階段（目前）：閾值設定為 `null`（僅警告模式）。設定 `max_errors: 0` 可啟用零錯誤品質閘門。
:::
