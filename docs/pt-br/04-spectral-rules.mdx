...
O projeto utiliza o [Spectral](https://stoplight.io/open-source/spectral) para linting OAS3 com uma arquitetura de duas configurações que separa o CI/ferramentas externas da lógica customizada do pipeline.
...
Estas regras são corrigidas automaticamente pelo reconciliador quando violações são detectadas. Consulte [Correções Aplicadas](/api-specs/pt-br/03-fixes-applied/) para detalhes sobre cada correção.
...
:::note
`f5-path-params` é uma regra com severidade de erro, mas não é corrigida automaticamente. Violações indicam um defeito genuíno na especificação que requer revisão manual.
:::
