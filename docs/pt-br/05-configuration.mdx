...
Os testes ao vivo resolvem seus domínios semânticos a partir da saída de transformação corrigida.
Os arquivos brutos em `specs/original/` permanecem como os bytes de entrada upstream imutáveis e
nunca são apresentados como o contrato de API intermediário corrigido.
...
| `fix_strategies` | Mapeia cada tipo de discrepância para sua ação de correção. Veja [Correções Aplicadas](/api-specs/en/03-fixes-applied/). |
