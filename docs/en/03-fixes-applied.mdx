...
These fixes address OAS3 quality issues found by [Spectral linting](/api-specs/en/04-spectral-rules/). They add or correct metadata without changing API behavior.
...
The server URL and variable values come from `spectral.servers` in [validation.yaml](/api-specs/en/05-configuration/).
...
Values come from `spectral.contact` in [validation.yaml](/api-specs/en/05-configuration/).
...
The scheme definition comes from `spectral.security_scheme` in [validation.yaml](/api-specs/en/05-configuration/). This tells API consumers how to authenticate but does not change runtime behavior.
