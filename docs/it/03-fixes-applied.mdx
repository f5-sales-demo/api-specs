...
Queste correzioni regolano i vincoli degli schemi OpenAPI affinché la specifica corrisponda a ciò che l'API live effettivamente applica. Ogni correzione è guidata da un oggetto `Discrepancy` prodotto durante la [validazione](/api-specs/it/02-pipeline/#2-validate).
...
:::caution
Le correzioni dei vincoli modificano il contratto API. Vengono applicate solo quando supportate da evidenze provenienti da Schemathesis o da test di validazione dei vincoli eseguiti contro l'API live.
:::
...
- **Tipi di dati** -- `type`, `format`

## Arricchimenti Spectral

Queste correzioni risolvono problemi di qualità OAS3 individuati dal [linting Spectral](/api-specs/it/04-spectral-rules/). Aggiungono o correggono metadati senza modificare il comportamento dell'API.

### Iniezione del blocco servers

**Regola**: `oas3-api-servers` | **Modifica del contratto**: No
...
L'URL del server e i valori delle variabili provengono da `spectral.servers` in [validation.yaml](/api-specs/it/05-configuration/).

### Iniezione delle informazioni di contatto

**Regola**: `info-contact` | **Modifica del contratto**: No
...
I valori provengono da `spectral.contact` in [validation.yaml](/api-specs/it/05-configuration/).

### Derivazione dei tag delle operazioni

**Regola**: `operation-tags` | **Modifica del contratto**: No
...
**Regola**: `oas3-unused-component` | **Modifica del contratto**: No
...
**Regola**: `operation-operationId-unique` | **Modifica del contratto**: No
...
**Regola**: `oas3-valid-schema-example` | **Modifica del contratto**: No
...
**Regola**: `no-script-tags-in-markdown` | **Modifica del contratto**: No
...
**Modifica del contratto**: No (solo documentazione)
...
La definizione dello schema proviene da `spectral.security_scheme` in [validation.yaml](/api-specs/it/05-configuration/). Questo indica ai consumatori dell'API come autenticarsi, ma non modifica il comportamento a runtime.

## Formattazione JSON

**Modifica del contratto**: No
...
1. **existing** -- Usa il vincolo della specifica esistente se è valido
2. **discovery** -- Usa il comportamento scoperto dall'API live
3. **inferred** -- Inferisci da pattern riscontrati in endpoint simili
