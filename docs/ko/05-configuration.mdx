---
title: 설정 참조
description: validation.yaml 및 endpoints.yaml 설정 파일에 대한 참조
i18n:
  sourceHash: fd479ebd3b67
  translator: machine
---
...
:::caution
`validation.yaml`의 `base_url`, `tenant`, `namespace` 값은 일반적인 플레이스홀더입니다. 파이프라인을 실행하기 전에 환경 변수를 통해 해당 환경에 맞게 **반드시** 설정해야 합니다:
...
라이브 테스트는 수정된 변환 출력에서 의미론적 도메인을 확인합니다.
`specs/original/`의 원본 파일은 변경 불가능한 업스트림 바이트로 남아 있으며
수정된 중간 API 계약으로 제공되지 않습니다.
...
| `fix_strategies` | 각 불일치 유형을 수정 작업에 매핑합니다. [적용된 수정 사항](/api-specs/en/03-fixes-applied/)을 참조하세요. |
...
4. 원하는 위치의 `test_order:`에 엔드포인트 이름을 추가합니다

:::tip
프레임워크는 `endpoints.yaml`에 나열된 항목과 관계없이 조정 과정에서 268개의 모든 스펙에 대해 유효성 검사를 수행합니다. 엔드포인트 설정은 Schemathesis를 사용한 라이브 API 테스트를 받을 리소스를 제어합니다.
:::
