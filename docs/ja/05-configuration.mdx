...
ライブテストは、修正された変換出力からセマンティックドメインを解決します。
`specs/original/` 内の生ファイルは不変の上流ランディングバイトのままであり、
修正された中間 API 契約として提示されることはありません。
...
| `fix_strategies` | 各不一致タイプを修正アクションにマッピング。[適用される修正](/api-specs/en/03-fixes-applied/)を参照。 |
