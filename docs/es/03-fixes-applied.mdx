...
Estas correcciones ajustan las restricciones del esquema OpenAPI para que la especificación coincida con lo que la API en vivo realmente aplica. Cada corrección es impulsada por un objeto `Discrepancy` producido durante la [validación](/api-specs/es/02-pipeline/#2-validate).
...
Estas correcciones abordan problemas de calidad OAS3 encontrados por el [linting de Spectral](/api-specs/es/04-spectral-rules/). Agregan o corrigen metadatos sin cambiar el comportamiento de la API.
...
La URL del servidor y los valores de las variables provienen de `spectral.servers` en [validation.yaml](/api-specs/es/05-configuration/).
...
Los valores provienen de `spectral.contact` en [validation.yaml](/api-specs/es/05-configuration/).
...
La definición del esquema proviene de `spectral.security_scheme` en [validation.yaml](/api-specs/es/05-configuration/). Esto indica a los consumidores de la API cómo autenticarse pero no cambia el comportamiento en tiempo de ejecución.
