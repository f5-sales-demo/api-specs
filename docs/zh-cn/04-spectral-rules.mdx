---
title: Spectral 检查规则
description: 采用双配置架构的 Spectral，包含基础 OAS 规则和自定义 f5-path-params 函数
i18n:
...
供 `scripts/spectral_lint.py`（发现模式和门控模式）使用。继承基础配置并添加自定义 `f5-path-params` 函数：
...
Spectral 内置的 `path-params` 规则会错误地标记 F5 XC API 中使用点号分隔参数名（如 `{metadata.namespace}` 和 `{metadata.name}`）的情况。内置规则无法解析参数名中的点号，因此会将其报告为未声明。
...
当检测到违规时，这些规则会由协调器自动修复。有关每项修复的详情，请参阅[已应用的修复](/api-specs/zh-cn/03-fixes-applied/)。
...
:::note
`f5-path-params` 是错误严重级别的规则，但不会自动修复。违规表示规范存在真正的缺陷，需要人工审查。
:::
