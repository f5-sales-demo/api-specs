...
# Installer les dépendances Spectral (requis pour les étapes de linting)
...
4. **Enregistrez-la** dans le dispatcher `_apply_spectral_fix` :
   ```python
   spectral_fixers = {
       # ... existing fixers ...
...
:::caution
Les fonctions personnalisées ne fonctionnent que dans `spectral-pipeline.yaml`. N'ajoutez pas `functionsDir` ou `functions` à `.spectral.yaml` -- cette configuration doit rester compatible avec Super-Linter et les autres outils externes.
:::
