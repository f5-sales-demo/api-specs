...
| `tenant` | Tenant-Name, der in API-Pfaden verwendet wird. |
...
| `stateful_testing` | Zustandsbehaftetes, linkbasiertes Testen über Operationen hinweg aktivieren. |
...
| `fix_strategies` | Ordnet jeden Diskrepanztyp seiner Korrekturmaßnahme zu. Siehe [Angewandte Korrekturen](/03-fixes-applied/). |
...
| `auto_fix` | Zuordnung von Regelname zu Boolean. `true` bedeutet, dass der Reconciler Verstöße behebt. |
| `gate.max_errors` | Maximal zulässige Fehler im Post-Reconcile-Gate. `null` deaktiviert die Prüfung. |
...
| `version_from` | Versionsquelle. `git` leitet die Version aus dem Datum der Spezifikationsmetadaten + Patchnummer ab. |
...
| `api_group` | API-Gruppenprefix (`config`, `web`, usw.). |
...
1. Finden Sie den Spezifikationsdateinamen in `specs/original/` (Format: `docs-cloud-f5-com.NNNN.*.ves-swagger.json`)
...
:::tip
Das Framework validiert während des Abgleichs alle 268 Spezifikationen, unabhängig davon, was in `endpoints.yaml` aufgeführt ist. Die Endpunkt-Konfiguration steuert, welche Ressourcen mit Schemathesis live API-Tests durchlaufen.
:::
