...
Essas correções ajustam as restrições do esquema OpenAPI para que a especificação corresponda ao que a API em produção realmente aplica. Cada correção é conduzida por um objeto `Discrepancy` produzido durante a [validação](/api-specs/pt-br/02-pipeline/#2-validate).
...
Essas correções tratam problemas de qualidade OAS3 encontrados pelo [linting Spectral](/api-specs/pt-br/04-spectral-rules/). Elas adicionam ou corrigem metadados sem alterar o comportamento da API.
...
A URL do servidor e os valores das variáveis vêm de `spectral.servers` em [validation.yaml](/api-specs/pt-br/05-configuration/).
...
Os valores vêm de `spectral.contact` em [validation.yaml](/api-specs/pt-br/05-configuration/).
...
A definição do esquema vem de `spectral.security_scheme` em [validation.yaml](/api-specs/pt-br/05-configuration/). Isso informa aos consumidores da API como autenticar, mas não altera o comportamento em tempo de execução.
