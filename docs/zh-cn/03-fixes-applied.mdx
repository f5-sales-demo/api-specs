...
这些修复调整 OpenAPI 模式约束，使规范与实际 API 的执行行为保持一致。每项修复由[验证](/api-specs/en/02-pipeline/#2-validate)阶段产生的 `Discrepancy` 对象驱动。
...
这些修复解决由 [Spectral 检查](/api-specs/en/04-spectral-rules/)发现的 OAS3 质量问题。它们添加或修正元数据，而不改变 API 行为。
...
服务器 URL 和变量值来自 [validation.yaml](/api-specs/en/05-configuration/) 中的 `spectral.servers`。
...
值来自 [validation.yaml](/api-specs/en/05-configuration/) 中的 `spectral.contact`。
...
安全方案定义来自 [validation.yaml](/api-specs/en/05-configuration/) 中的 `spectral.security_scheme`。这告知 API 消费者如何进行身份验证，但不改变运行时行为。
